...
| **`features_version` not incremented after deploy with new features** | Startup lifespan computes a hash of sorted feature keys, compares to `features_registry_hash` in Redis. If changed, bulk-sets `features_version = NOW_MS` for all active tenants and update the hash. |
...
If the migration or feature flag system causes a production regression, follow this strict atomic order. Failure to follow the sequence will result in HTTP 500 errors.

1. **Step 1: Application Revert.** 
   - Remove all `require_feature` decorators from routes.
   - Remove all `<FeatureGate>` components or set them to a passthrough mode.
   - Deploy code. Verify traffic passes without feature checks.
2. **Step 2: Database Revert.** 
   - Run the Alembic down-migration.
   - Note: The `audit_log` or `UserActivity` extensions MUST NOT be dropped. Preserve these for compliance.
3. **Step 3: Cache Purge.** 
   - Execute Redis `FLUSHDB` or specific pattern match `DEL persona:*` and `DEL superadmin:*`.
4. **Step 4: Registry Sync.** 
   - Ensure `features_registry_hash` is cleared so that the next deployment triggers a fresh hash computation.
...
# ADDENDUM: High-Level System Architecture (v0 Hardening)

### Distributed State Synchronization Protocol
To ensure consistency across the distributed system (FastAPI nodes and Redis Cluster), the following protocol must be enforced for all configuration changes:
1. **DB Update:** Commit the change to the SQL database (Persona or Override).
2. **Version Bump:** Increment the `Tenant.features_version` using the DB server's clock (epoch ms).
3. **Invalidation Fan-out:** Emit a Redis `SREM`/`DEL` operation. For Persona updates, this involves iterating the `persona:tenants:{id}` set and deleting individual `persona:features:{tid}` keys.
4. **Consistency Model:** The system follows an "Eventually Consistent with Read-Repair" model. If the fan-out fails, the 5-minute TTL on `persona:features:*` provides an upper bound on staleness.

### Frontend Synchronization Interceptor (Axios/Fetch)
The frontend must implement a global interceptor that reads the `X-Features-Version` header. 
- **Rule:** If `Header.features_version > Redux.features_version`, the client MUST dispatch a background `refresh_features` action.
- **Optimistic Rendering:** During the background refresh, the UI must NOT block user interaction; it should use the stale feature set until the new one arrives to prevent "layout jump."

### Fallback Priority Specification
The resolution engine must follow this exact search order:
1. **Request State:** Local in-memory cache for the current execution thread.
2. **Redis Hash:** `persona:features:{tenant_id}`.
3. **Tenant Override DB:** `Tenant.feature_overrides`.
4. **Persona DB:** `PersonaConfiguration.features`.
5. **Static Registry:** `AVAILABLE_FEATURES[key]["default_enabled"]`.
6. **Hardcoded Safety:** `False` (Fail-Closed).

---

# SECTION 14: Deep Technical Specification (v1 Architectural Hardening)

### 14.1. X-Features-Version Middleware Design
The FastAPI middleware must be highly optimized to minimize latency on every authenticated request.
- **Implementation Pattern:**
  ```python
  @app.middleware("http")
  async def inject_features_version_header(request: Request, call_next):
      # 1. Skip unauthenticated or public routes
      if not request.state.user or not request.state.user.tenant_id:
          return await call_next(request)
          
      # 2. Prefer request.state cache if require_feature already ran
      version = getattr(request.state, "features_version", None)
      if version is None:
          # 3. Fast lookup from Redis or DB
          version = await FeatureService.get_tenant_version(request.state.user.tenant_id)
          request.state.features_version = version
          
      response = await call_next(request)
      response.headers["X-Features-Version"] = str(version)
      return response
  ```
- **Constraint:** Use a Redis Hash `persona:meta:{tenant_id}` to store the version for O(1) retrieval without fetching the full feature set.

### 14.2. Atomic Reassignment Lua Script (Redis Cluster Safe)
Redis `MULTI`/`EXEC` fails in clustered environments if keys map to different hash slots. A Lua script ensures atomicity.
- **Script:**
  ```lua
  -- KEYS[1]: Old Persona Set (persona:tenants:{old_id})
  -- KEYS[2]: New Persona Set (persona:tenants:{new_id})
  -- KEYS[3]: Tenant Feature Cache (persona:features:{tid})
  -- ARGV[1]: Tenant ID
  ...
  return true
  ```
- **Rationale:** This script must be executed within the `TenantService.update_persona` method after the DB transaction commits.

### 14.3. Circular Dependency DFS Specification
Startup validation in `features.py` must use Depth-First Search (DFS) to detect cycles.
- **Logic:**
  1. Build adjacency list from `AVAILABLE_FEATURES`.
  2. For each node, perform DFS.
  3. Maintain a `visited` set and a `recursion_stack` set.
  4. If a node is in `recursion_stack`, a cycle exists. Raise `RuntimeError`.
- **Pre-computation:** The `FEATURE_DEPENDENCY_CLOSURE` must be flattened such that `closure["A"]` contains every descendant dependency, not just direct ones.

---

# SECTION 15: Failure Modes & Resilience Matrix (v2 Hardening)

### 15.1. Distributed Failure Scenarios
| Failure Point | Impact | Behavior (Hardened) |
| :--- | :--- | :--- |
| **Redis Partial Outage** | Cache misses, high DB load. | **Circuit Breaker:** If Redis connection times out 3x in 10s, open circuit and bypass Redis for 60s. Log as `CRITICAL`. |
| **Worker Invalidation Failure** | Stale feature sets served to users. | **TTL Safety Net:** 5-minute TTL on all feature keys ensures staleness is bounded even if the invalidation fan-out crashes. |
| **DB Primary Down (Read-Only)** | Persona updates fail; `features_version` bump fails. | **Graceful Degraded Mode:** Allow read-only access using cached features. Superadmin UI shows `System in Maintenance`. |
| **Redis Flush (All data lost)** | Massive thundering herd on DB. | **Staged Warming:** Background task repopulates caches in priority order (active sessions first). See Section 16. |
| **Registry Code/DB Drift** | Code expects Key X, DB lacks it. | **Absence-means-Enabled:** The "Absent Key Fallback" (Section 1) prevents 500 errors or hidden features during partial deployments. |

### 15.2. Invalidation Fan-Out Failure Protocol
When a Persona is updated, and the Redis fan-out (`DEL persona:features:{tid}`) for 5,000 tenants fails midway:
- **Detection:** Log the number of successful vs. failed deletions.
- **Recovery:** Do NOT retry individual keys synchronously. Instead, delete the `persona:tenants:{id}` set and the `features_registry_hash`. This forces a rebuild of the reverse-index and staged warming on next access.

---

# SECTION 16: Cold-Start DB Stampede Mitigation Protocol

To prevent the **Thundering Herd** problem when Redis is empty or the registry hash changes:

### 16.1. Staged Cache Warming Pattern
Instead of letting every request trigger an individual DB query, the system must use a background warmer:
1. **Identify Active Tenants:** Query for tenants with a session in the last 24 hours.
2. **Batch Resolution:** Fetch merged feature sets in batches of 50.
3. **Pipeline Writes:** Use Redis Pipelining to push all 50 feature sets in a single round-trip.
4. **Jittered TTL:** Add random jitter (±60s) to the 5m TTL during warming to prevent a second thundering herd when the warmed caches expire simultaneously.

### 16.2. Sentinel Versioning
Before resolution, `require_feature` checks a local LRU cache (1-minute TTL) for the `features_registry_hash`. If it differs from the module-level hash, it triggers a `WarmingRequired` signal to the background task queue and forces the current request to DB-fallback mode.

---

# SECTION 17: Concurrent Configuration Update Protocol (v3 Distributed Hardening)

### 17.1. Optimistic Locking on Persona Mutations
To prevent "Lost Updates" when two Superadmins edit the same Persona simultaneously, the API must implement ETag or Version-based concurrency control.
- **Requirement:** Add an `version_id` column (Integer) to `PersonaConfiguration`.
- **Protocol:**
  1. Client `GET`s Persona (includes `version_id`).
  2. Client `POST`s update (includes `version_id`).
  3. Server executes: `UPDATE persona_configurations SET features = :f, version_id = version_id + 1 WHERE id = :id AND version_id = :old_version`.
  4. If `rowcount == 0`, return `409 Conflict`.

### 17.2. Distributed Lock for Bulk Invalidation
Massive invalidation tasks (Section 5) must not run concurrently on multiple FastAPI worker nodes.
- **Logic:**
  ```python
  async with RedisLock(f"lock:invalidate:persona:{pid}", ttl=60):
      # Execute fan-out invalidation...
  ```
- **Safety:** If a node dies holding the lock, the 60s TTL ensures the system eventually recovers.

---

# SECTION 18: Redis Cluster Hash Slot Optimization Strategy

### 18.1. Key Tagging for Atomic Execution
To ensure the Lua script (Section 14.2) works in a Redis Cluster, all keys for a specific tenant MUST reside on the same hash slot.
- **Naming Convention:** Use Redis "Key Tags" `{tenant_id}`.
- **Optimized Keys:**
  - `persona:features:{123}`
  - `persona:meta:{123}`
  - `persona:audit:{123}`
- **Constraint:** Reverse-index sets like `persona:tenants:{persona_id}` CANNOT be tagged by tenant_id, meaning atomic reassignment (Section 14.2) across sets and individual tenant keys requires careful cross-slot handling or a multi-stage atomic cleanup.

---

# SECTION 19: Zero-Downtime Migration Blueprint (v4 Data Hardening)

### 19.1. Column Addition Protocol
The migration of the `tenants` table must avoid long-running locks that block the application.
1. **Pre-Deployment:** Add `persona_id` and `features_version` as nullable columns with no default.
2. **Backfill Task:** Run the batched update (Section 1) in a loop using a background script, not the Alembic migration itself.
3. **Registry Seed:** Seed the `default` persona with an empty features dict.
4. **Final Step:** Once 100% of tenants have a `persona_id`, deploy the application code containing the `require_feature` decorators.

### 19.2. Registry Hash Invalidation Guard
A deployment with new features must not trigger a massive DB update during the critical first minutes of a rollout.
- **Strategy:** Use a "Lazy Bump" strategy. Instead of a bulk DB update on startup, mark the `features_registry_hash` as "dirty" in Redis. Individual tenants are bumped to the new version only when they first hit a `require_feature` miss.

---

# SECTION 20: Point-in-Time Audit Recovery Protocol

### 20.1. Snapshot Integrity Guarantee
The "Resolved Feature Set" stored in the audit log (Section 1) is the only source of truth for historical compliance.
- **Integrity Rule:** Audit records must be write-only. No application role (including Superadmin) may have `UPDATE` or `DELETE` permissions on the audit table.
- **Pruning Safety:** The background pruning task (Section 3) must verify that the "Last Compliance Date" (stored in Redis) is older than the records being deleted.
- **Format:** Always store the snapshot as a canonical JSON string (sorted keys) to ensure MD5 hash consistency if audit integrity is verified later.

---

# SECTION 21: Redis Pipeline & Lua Performance Benchmarking (v5 Performance Hardening)

### 21.1. Pipeline Batch Size Optimization
Bulk invalidations (Section 5) must use Redis Pipelining to avoid network round-trip overhead.
- **Benchmark Target:** Resolution of 1,000 tenants in < 100ms.
- **Algorithm:**
  ```python
  async with redis.pipeline() as pipe:
      for tid in affected_tenants:
          pipe.delete(f"persona:features:{{{tid}}}")
          if len(pipe) >= 500:
              await pipe.execute()
  ```
- **Note:** Large pipelines (>1000 commands) can block the Redis event loop. Stick to 500-unit batches.

### 21.2. Lua Script execution cost
Lua scripts (Section 14.2) are atomic but block other commands.
- **Limit:** Lua execution must not exceed 5ms.
- **Monitoring:** Enable Redis `slowlog` with a 10ms threshold to identify "Heavy Persona Updates."

---

# SECTION 22: CDN Edge Gating Strategy

### 22.1. JWT Claims vs. Custom Header
For static assets or cached API responses, the CDN must be able to gate content without hitting the origin.
- **Protocol:**
  1. **Option A (v5 Choice):** Include a truncated feature-hash in a custom `X-Features-Hash` cookie.
  2. **Option B (Future):** Add top-level feature flags to the JWT (max 5 critical flags) to avoid session invalidation overhead.
- **Staleness Mitigation:** If the Edge receives a request with an `X-Features-Version` older than the last known global invalidation, it MUST force a `revalidate` request to the origin.

### 22.2. Edge Invalidation Hook
Origin FastAPI servers should emit a "Cache-Tag" invalidation to the CDN when a Persona is updated, ensuring Edge caches are cleared for all affected industry segments instantly.

---

# SECTION 23: Multi-Tenant Key Leakage Prevention (v6 Security Hardening)

### 23.1. Redis Key Isolation (Key Tags & ACLs)
To prevent Tenant A from accessing Tenant B's feature set via common exploitation vectors (e.g., CRLF injection in keys), the Redis layer must enforce strict isolation.
- **Key Tagging Requirement:** Every feature-related key MUST be wrapped in `{tenant_id}` tags (e.g., `persona:f:{123}`). This ensures that even in Redis Cluster, all keys for one tenant are co-located on one node, while also acting as a semantic barrier.
- **ACL Scoping:** Application connections to Redis should use a dedicated user with `~persona:f:*` access only, preventing feature-gating code from interacting with unrelated session or tier data.

### 23.2. Response Smuggling Prevention
The `X-Features-Version` header (Section 14.1) must be strictly validated.
- **Constraint:** If the header contains non-numeric characters or exceeds 16 digits (max epoch ms), the middleware must drop it and log a `SECURITY_ALERT`. This prevents header-smuggling attacks where malicious actors inject metadata into response streams.

---

# SECTION 24: Audit Log Cryptographic Integrity Protocol

### 24.1. Audit Record Signing
To prevent "Ghost Edits" to the audit trail by actors with DB root access, each audit entry (Section 1) must be cryptographically signed.
- **Protocol:**
  1. Concatenate: `actor_id + tenant_id + timestamp + before_blob + after_blob`.
  2. Compute HMAC-SHA256 using a `SYSTEM_AUDIT_SECRET`.
  3. Store the signature in an `integrity_hash` column.
- **Verification:** A nightly background task must re-compute hashes for the last 1,000 records and alert if a mismatch is found (indicating DB-level tampering).

---

# FUTURE CODING GUIDELINES (v7 Maintainability Hardening)

### Feature Lifecycle & Graduation
- **Zombie Flag Prevention:** Every new feature should have a proposed "graduation date" or criteria (e.g., 100% adoption for 3 months).
- **Cleanup SOP:** Once graduated, the flag logic and `<FeatureGate>` wrappers should be removed from the codebase.
- **Mandatory PR Check:** Use the CLI `audit-codebase` command in the CI pipeline to block PRs that introduce a feature key not present in `features.py`.

### Background task proctoring gap
APScheduler jobs are not HTTP-request-scoped and cannot use `Depends(require_feature(...))`. Call `get_tenant_features_sync(tenant_id)` explicitly inside any scheduled job that has a feature dependency. Do not assume all features are enabled inside a scheduled job — this is especially critical for proctoring-related jobs where a tenant may not have proctoring enabled.

### User-Level Overrides
User-level gating (e.g., for individual beta testers) is explicitly out of scope for v1. All gating is currently Tenant-level via Personas and Overrides. However, the architecture is designed to extend to `User.feature_overrides` by adding a third layer to the merge logic: `overrides[user]` > `overrides[tenant]` > `persona` > `default_enabled`.

---

# SECTION 25: Feature-Flag CLI Tool Deep Specification

To reduce developer friction and prevent "Ghost Keys," a dedicated CLI tool must be provided.

### 25.1. Command: `scaffold-feature`
- **Arguments:** `--key`, `--display-name`, `--category`.
- **Action:** 
  1. Append entry to `backend/core/constants/features.py`.
  2. Append translation stubs to all 11 JSON files in `frontend/src/i18n/locales/`.
  3. Emit a `git add` command for the affected files.

### 25.2. Command: `audit-flags`
- **Action:** 
  1. Parse `features.py` for registered keys.
  2. Grep `frontend/src` and `backend/` for `require_feature` or `FeatureGate` usage.
  3. Report "Dead Flags" (Registered but unused) and "Zombie Flags" (Enabled 100% in all personas for >30 days).

---

# SECTION 26: Persona Mocking & Testing Framework

### 26.1. Unit Test Fixture Factory
Provide a utility for `pytest` to easily generate mocked feature sets.
- **Usage:**
  ```python
  @pytest.mark.asyncio
  async def test_live_quiz_logic(tenant_with_features={"live_quiz": True}):
      # Logic here...
  ```
- **Rationale:** This replaces the need for actual DB/Redis state in lower-level unit tests, drastically reducing test execution time.

### 26.2. Frontend Storybook Integration
The `<FeatureGate>` component must support a `mockFeatures` prop that is active ONLY when `process.env.STORYBOOK === 'true'`. This allows designers to verify industry-specific terminology variants in isolation.

---

# SECTION 27: Prometheus Metrics & Alerting (v8 Operability Hardening)

To ensure the resolution engine is performing within the latency budget (< 5ms), the following metrics must be exposed.

### 27.1. Key Performance Indicators (KPIs)
- `ff_cache_hit_total{tenant_id}`: Counter for Redis hits.
- `ff_cache_miss_total{tenant_id}`: Counter for DB fallbacks.
- `ff_resolution_latency_seconds`: Histogram with buckets `[0.001, 0.005, 0.010, 0.050, 0.100]`.
- `ff_active_zombie_flags`: Gauge tracking keys enabled in all personas but still present in code.

### 27.2. Critical Alerts
- **High Fallback Rate:** Alert if `cache_miss / (cache_hit + cache_miss) > 10%` over 5 minutes.
- **Slow Resolution:** Alert if `p99(ff_resolution_latency_seconds) > 50ms`.

---

# SECTION 28: Distributed Tracing & OpenTelemetry Integration

Every feature-gated check must be visible in the request trace to facilitate 3 AM debugging.

### 28.1. Span Attributes
The `require_feature` dependency must inject attributes into the active OpenTelemetry span:
- `ff.key`: The feature being checked.
- `ff.result`: Boolean (Allowed/Denied).
- `ff.origin`: (Cache | DB | Registry).
- `ff.tenant_id`: The ID of the affected tenant.

### 28.2. Trace Propagation logic
If the frontend triggers a background refresh (Section 4), the `X-Trace-Id` must be passed to the `/me` endpoint to link the UI-detected staleness to the backend resolution logic.

---

# SECTION 29: Effective Permissions Debugger Technical Specification

The `get_effective_features(tenant_id)` command (Section 13) must output the following diagnostic table:

| Feature Key | Status | Source | Reason |
| :--- | :--- | :--- | :--- |
| `live_quiz` | ENABLED | Override | Explicitly enabled for Tenant 123 |
| `realtime_ws` | DISABLED | Persona | Disabled in 'K12_Basic' Persona |
| `proctoring` | DISABLED | Prerequisite | Failed because `realtime_ws` is DISABLED |

- **Protocol:** This utility must bypass the Redis cache and perform a "Deep Resolve" against the DB and Registry to identify drift.

---

# SECTION 30: Final Production Implementation Checklist

Before the `DISABLE_FEATURE_GATING` environment variable is unset in production, the following must be verified.

### 30.1. Performance & Scale
- [ ] Redis Pipelining (Section 21) verified with 1,000 parallel resolution requests.
- [ ] Resolution latency P99 < 10ms on hit, < 30ms on miss.
- [ ] Jitter algorithms (Section 16) confirmed to be ±60s via logging.

### 30.2. Security & Compliance
- [ ] Audit logs (Section 3/24) verified for cryptographic integrity.
- [ ] Key isolation (Section 23) confirmed via Redis `MONITOR` (no leakage).
- [ ] `super_admin` short-circuit (Section 2) tested against a broken persona.

### 30.3. Observability
- [ ] Prometheus metrics (Section 27) visible in Grafana.
- [ ] OpenTelemetry spans (Section 28) correctly show `ff.key` attributes.

---

# SECTION 31: Operational Runbooks (3 AM Responses)

### 31.1. Scenario: Redis Latency Spike
1. **Detection:** Alert `Slow Resolution` fires.
2. **Action:** Enable `DISABLE_FEATURE_GATING=true`. This short-circuits all checks (Section 2).
3. **Investigation:** Check `ff_cache_miss_total`. If misses are high, trigger `audit-flags` (Section 25) to check for ghost keys causing mass invalidations.

### 31.2. Scenario: Default Persona Corruption
1. **Detection:** Platform returns 500 on all gated routes.
2. **Action:** Execute `python -m cli reset-default-persona`. This seeds the `default` persona with an empty features dict, triggering code-level fallbacks (Section 9).

### 31.3. Scenario: Audit Tampering Detected
1. **Detection:** Nightly integrity task (Section 24) alerts on HMAC mismatch.
2. **Action:** Freeze all Persona updates. Compare `integrity_hash` with database binary logs to identify the malicious actor.

---

# FINAL STATUS: PRODUCTION HARDENED
This document (v9) is the definitive technical source for the Swaya.me Industry Persona and Feature Flag architecture. All layers (DB, Cache, API, Frontend, Security, and Ops) are fully specified and interlocked.

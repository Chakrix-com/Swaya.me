#!/usr/bin/env bash
# deploy_template.sh — Swaya.me CI/CD menu (template for contributors)
#
# ─── SETUP INSTRUCTIONS ───────────────────────────────────────────────────────
#
#  1. Copy this file to deploy.sh:
#       cp deploy_template.sh deploy.sh && chmod +x deploy.sh
#
#  2. Fill in the variables in the "Config" section below with paths
#     that match YOUR server setup.
#
#  3. deploy.sh is gitignored — your server paths stay private.
#
# ──────────────────────────────────────────────────────────────────────────────
#
# Usage:
...
#
# Commands:
#   deploy-test       Build & deploy to your test environment
#   promote-live      Tag + promote test build → production
#   rollback-live     Restore a previous release on production
...
# ─── Config — SET THESE TO MATCH YOUR SERVER ─────────────────────────────────

# Root of your local git working directory (where you run this script from)
DEV_ROOT="/path/to/your/project"

# Root directory where your production backend code lives
LIVE_ROOT="/path/to/your/live/root"

# Directory served by nginx for your production frontend (the built dist/)
LIVE_FRONTEND="/path/to/your/live/frontend/webroot"

# Directory for release backups (will be created if it doesn't exist)
BACKUP_DIR="/path/to/your/backups"

# Full path to the nginx binary on your server
NGINX_BIN="/path/to/nginx/sbin/nginx"

# systemd service names for test and live backends
TEST_SERVICE="your-app-test.service"
LIVE_SERVICE="your-app-live.service"

# Port your test backend listens on
TEST_PORT=8001

# Port your live backend listens on
LIVE_PORT=8000

# Hostname for your test environment (used for health checks and display)
TEST_HOST="test.yourdomain.com"

# Hostname for your live/production environment
LIVE_HOST="www.yourdomain.com"
...
cmd_deploy_test() {
    ...
    sleep 2
    health_check "$TEST_HOST" "$TEST_PORT" "$TEST_HOST"
    success "Test deploy complete → https://$TEST_HOST"
}

# ─── Command: promote-live ───────────────────────────────────────────────────
cmd_promote_live() {
    header "Promote → live ($LIVE_HOST)"
    ...
    warn "This will update PRODUCTION ($LIVE_HOST)."
    ...
    # 8. Write deployed version marker
    ...
    if ! health_check "$LIVE_HOST" "$LIVE_PORT" "$LIVE_HOST"; then
    ...
    success "Live promotion complete → https://$LIVE_HOST"
...
cmd_migrate_test() {
    header "Alembic migrate → test DB"
...
cmd_migrate_live() {
    header "Alembic migrate → live DB"
...
cmd_health() {
    ...
    health_check "$TEST_HOST" "$TEST_PORT" "$TEST_HOST" || true
    health_check "$LIVE_HOST" "$LIVE_PORT" "$LIVE_HOST" || true
...
show_menu() {
    ...
    echo ""
    echo -e "  ${GREEN}1)${RESET} Deploy → $TEST_HOST       (build + restart test)"
    echo -e "  ${YELLOW}2)${RESET} Promote → $LIVE_HOST  (tag + push to production)"
